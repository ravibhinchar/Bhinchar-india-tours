...
export function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // On successful registration, our backend returns the user and a token
      if (response.data.token && response.data.user) {
        login(response.data.user, response.data.token);
        navigate("/profile"); // Redirect to dashboard
      } else {
        setError("Registration successful, however login failed. Please go to Login page.");
      }
    } catch (err: any) {
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Button
              type="submit"
              disabled={isLoading}
              className="w-full bg-gradient-to-r from-orange-500 to-pink-600 hover:from-orange-600 hover:to-pink-700 text-white py-6 text-lg rounded-xl flex items-center justify-center"
            >
              {isLoading ? (
                "Creating Account..."
              ) : (
